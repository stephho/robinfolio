...
class notion: 
    """
    Class for interacting with Notion API 

    Attributes: 
        notion_token (str): Notion API token
        notion_header (dict): HTTP headers required by Notion API 
        db_base_url (str): Notion's API URL for interacting with database objects
        page_base_url (str): Notion's API URL for interacting with page objects
    """
    ...
    def __init__(self, notion_token):
        ...
        self.db_base_url = 'https://api.notion.com/v1/databases'
        self.page_base_url = 'https://api.notion.com/v1/pages'


    def get_db_schema(self, db_id, simple=False): 
        """
        ...
        """
        db_url = self.db_base_url + '/' + db_id 
        response = requests.get(db_url, headers=self.notion_header)
    ...
    def get_prop_id(self, db_id, prop_name=None): 
        """
        ...
        """
        db_schema = self.get_db_schema(db_id=db_id)
    ...
    def get_db_pages(self, db_id, filters=None): 
        """
        ...
        """
        db_query_url = self.db_base_url + '/' + db_id + '/query'
        ...
        response = requests.post(db_query_url, headers=self.notion_header, data=filters_data)
        response = json.loads(response.text)
        ...
        if response['object'] == 'error': 
            ...
            while next_page: 
                ...
                response = requests.post(db_query_url, headers=self.notion_header, data=next_page_data)
                response = json.loads(response.text)
    ...
    def get_db_pg_ids(self, db_id): 
        ...
        db_schema = self.get_db_schema(db_id=db_id, simple=True)
        prop_title = []
        ...
        pages = self.get_db_pages(db_id=db_id)
        pg_dict = {}
        for pg in pages: 
    ...
    def create_db_pg_template(self, db_id, pg_icon=None): 
        """
        ...
        db_schema = self.get_db_schema(db_id, simple=True)
    ...
    def get_prop_value(self, pg_id, prop_id): 
        """
        ...
        """
        prop_url = self.page_base_url + '/' + pg_id + '/properties/' + prop_id 
        response = requests.get(prop_url, headers=self.notion_header)
        response = json.loads(response.text)
        ...
        if response['object'] == 'error': 
            ...
            if prop_type == 'rollup': 
                next_page = response['has_more']
                while next_page: 
                    next_page_url = self.page_base_url + '/' + pg_id + '/properties/' + prop_id + '?start_cursor=' + response['next_cursor'] 
                    response = requests.get(next_page_url, headers=self.notion_header)
                    response = json.loads(response.text)
                    if response['object'] == 'error': 
                        print('failed to get value of property ID {}: {}'.format(prop_id, response['message']))
                    else: 
    ...
    def create_db_pg(self, create_data): 
        """
        ...
        response = requests.post(self.page_base_url, headers=self.notion_header, data=create_json)
        response = json.loads(response.text)
    ...
    def update_db_pg(self, pg_id, update_dict): 
        """
        ...
        """
        pg_url = self.page_base_url + '/' + pg_id 

        # get parent db id 
        response = requests.get(pg_url, headers=self.notion_header)
        pg_db_id = json.loads(response.text)['parent']['database_id']

        # get parent db schema 
        db_schema = self.get_db_schema(db_id=pg_db_id, simple=True)
        ...
        update_response = requests.patch(pg_url, headers=self.notion_header, data=update_json)
