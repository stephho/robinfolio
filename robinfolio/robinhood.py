...
def get_stock_name(instrument_id): 
    """
    Get a stock's ticker symbol and name, given Robinhood's instrument ID for that stock 
    ...
    Example: 
        For Apple: 'AAPL' is the stock ticker symbol, 'Apple' is the stock name,
        and the instrument ID is '450dfc6d-5510-4d40-abfb-f633b7d9be3e'
    """
...
def get_order_history(instrument_id=None, stock_ticker=None): 
    ...
    Args:  
        instrument_id (str): Optional filter for filled orders for a particular stock, by its Robinhood instrument ID
            Example: Apple is '450dfc6d-5510-4d40-abfb-f633b7d9be3e' 
        stock_ticker (str): Optional filter for filled orders for a particular stock ticker symbol
            Example: 'AAPL' is the stock ticker symbol for Apple. Not case sensitive 
    ...
    if instrument_id == None and stock_ticker != None: 
        instr_id, stock_name = get_instrument_id(stock_ticker=stock_ticker) 
        if instr_id == None: 
            return None
    else: 
